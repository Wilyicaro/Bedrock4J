...
import wily.bedrock4j.client.screen.ConfirmationScreen;
import wily.bedrock4j.client.screen.BedrockLoadingScreen;
import wily.bedrock4j.client.screen.OverlayPanelScreen;
...
public class SDLControllerHandler implements Controller.Handler{
        ...
        if (!init) {
            ...
            if (!nativesFile.exists()){
                if (!(minecraft.screen instanceof OverlayPanelScreen) && MinecraftAccessor.getInstance().hasGameLoaded()) {
                    ...
                    BedrockOptions.selectedControllerHandler.set(GLFWControllerHandler.getInstance());
                    BedrockOptions.selectedControllerHandler.save();
                    minecraft.setScreen(new ConfirmationScreen(s, Component.translatable("bedrock4j.menu.download_natives",getName()), Controller.Handler.DOWNLOAD_MESSAGE, b -> {
                        Stocker<Long> fileSize = new Stocker<>(1L);
                        BedrockLoadingScreen screen = new BedrockLoadingScreen(Controller.Handler.DOWNLOADING_NATIVES, CommonComponents.EMPTY){
                            @Override
                            public void tick() {
                                if (getProgress() == 100) {
                                    BedrockOptions.selectedControllerHandler.set(getInstance());
            ...
            if (!SdlInit.SDL_Init(SdlSubSystemConst.SDL_INIT_JOYSTICK | SdlSubSystemConst.SDL_INIT_GAMEPAD)) {
                Bedrock4J.LOGGER.warn("SDL Game Controller failed to start!");
                return;
            }
    ...
    public void applyGamePadMappingsFromBuffer(BufferedReader reader) {
        ...
        Bedrock4J.LOGGER.warn("Added SDL Controller Mappings: {} Code", i);
    }
