...
import wily.bedrock4j.mixin.base.MouseHandlerAccessor;
...
public class ControllerManager {
    ...
    public static final Component CONTROLLER_DETECTED = Component.translatable("bedrock4j.controller.detected");
    public static final Component CONTROLLER_DISCONNECTED = Component.translatable("bedrock4j.controller.disconnected");


    public static Controller.Handler getHandler() {
        return BedrockOptions.selectedControllerHandler.get();
    }
    ...
    public static void updatePlayerCamera(BindingState.Axis stick, Controller controller){
        ...
        double f = Math.pow(BedrockOptions.controllerSensitivity.get() * (double)0.6f + (double)0.2f,3) * 7.5f * (minecraft.player.isScoping() ? 0.125: 1.0);
        minecraft.player.turn(getCameraCurve(stick.getSmoothX()) * f, getCameraCurve(stick.getSmoothY()) * f * (BedrockOptions.invertYController.get() ? -1 : 1));
    }

    public static float getCameraCurve(float f){
        if (BedrockOptions.linearCameraMovement.get()) return f;
        ...
        CompletableFuture.runAsync(()-> new Timer().scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                if (minecraft.isRunning() && getHandler().update()) {
                    ...
                    if (!getHandler().isValidController(BedrockOptions.selectedController.get())) {
                        if (connectedController != null) connectedController.disconnect(ControllerManager.this);
                        return;
                    }
                    if (connectedController == null && (connectedController = getHandler().getController(BedrockOptions.selectedController.get())) != null) connectedController.connect(ControllerManager.this);
                    minecraft.execute(() -> {
            ...
            Bedrock4J.LOGGER.warn(t.getMessage());
            return null;
        });
    }

    public void setPointerPos(double x, double y){
        setPointerPos(x, y, isControllerTheLastInput() && BedrockOptions.controllerVirtualCursor.get());
    }
    ...
    public synchronized void updateBindings(Controller controller) {
        ...
        for (ControllerBinding<?> binding : ControllerBinding.map.values()) {
            ...
            if (minecraft.player != null && minecraft.getConnection() != null && controller.hasLED() && minecraft.getConnection().getPlayerInfo(minecraft.player.getUUID()) instanceof LegacyPlayerInfo i){
                float[] color = Bedrock4JClient.getVisualPlayerColor(i);
            ...
            s : if (minecraft.screen != null) {
                ...
                ControllerBinding<?> cursorBinding = LegacyKeyMapping.of(Bedrock4JClient.keyToggleCursor).getBinding();
    ...
    public boolean allowCursorAtFirstInventorySlot(){
        return (isControllerTheLastInput() && BedrockOptions.controllerCursorAtFirstInventorySlot.get()) || (!isControllerTheLastInput() && BedrockOptions.cursorAtFirstInventorySlot.get());
    }
    ...
    public void toggleCursor(){
        setCursorMode(BedrockOptions.CursorMode.values()[Stocker.cyclic(0, getCursorMode().ordinal() + 1, BedrockOptions.CursorMode.values().length)]);
        updateCursorMode();
    }
    ...
    public BedrockOptions.CursorMode getCursorMode() {
        return BedrockOptions.cursorMode.get();
    }

    public void setCursorMode(BedrockOptions.CursorMode cursorMode) {
        BedrockOptions.cursorMode.set(cursorMode);
        BedrockOptions.cursorMode.save();
    }
    ...
    public void updateCursorInputMode(){
        if (!minecraft.mouseHandler.isMouseGrabbed()) setCursorInputMode(!BedrockOptions.hasSystemCursor());
    }
