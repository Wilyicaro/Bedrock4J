...
import wily.bedrock4j.client.screen.ScrollableRenderer;
...
public record GlobalPacks(List<String> list, boolean applyOnTop) {
    ...
    public static final FactoryConfig.StorageHandler STORAGE = new FactoryConfig.StorageHandler().withFile("bedrock4j/global_packs.json");
    ...
    public static class Selector extends AbstractWidget implements ControlTooltip.ActionHolder {
        ...
        protected void renderWidget(GuiGraphics guiGraphics, int i, int j, float f) {
            ...
            for (int index = 0; index < displayPacks.size(); index++) {
                ...
                if (model.selected.contains(displayPacks.get(scrolledList.get() + index)))  FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.PACK_SELECTED, getX() + 20 + 30 * index,getY() +font.lineHeight + 3,30,30);
                if (scrolledList.get() + index == selectedIndex)
                    FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.PACK_HIGHLIGHTED, getX() + 20 + 30 * index,getY() +font.lineHeight + 3,30,30);
                visibleCount++;
            }
