...
import wily.bedrock4j.client.screen.JoinGameScreen;
import wily.bedrock4j.client.screen.ServerRenderableList;
                ...
                addRenderable(onlineButton = new AbstractButton(0, 0, 270, 30, Component.literal(profile.getName())) {
                    ...
                    protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float f) {
                        ...
                        if (minecraft.options.touchscreen().get().booleanValue() || isHovered) {
                            ...
                            int u = mouseX - getX();
                            FactoryGuiGraphics.of(guiGraphics).blitSprite(u < 30 && u > 5 ? BedrockSprites.JOIN_HIGHLIGHTED : BedrockSprites.JOIN, getX(), getY(), 32, 32);
                        }
                    }
