...
public interface ControlTooltip {
    ...
    static Component getMenuOffAction(LegacyMenuAccess<?> a){
        ...
        if (a.getHoveredSlot() != null){
            if (a.getMenu().getCarried().isEmpty()) {
                ...
                if (a.getHoveredSlot().hasItem() && FactoryAPIClient.hasModOnServer && Bedrock4J.canRepair(a.getHoveredSlot().getItem(),a.getMenu().getCarried())) return LegacyComponents.REPAIR;
                if (a.getHoveredSlot().hasItem() && FactoryAPIClient.hasModOnServer && Bedrock4J.isDyeableItem(a.getHoveredSlot().getItem().getItemHolder()) && a.getMenu().getCarried().getItem() instanceof DyeItem) return LegacyComponents.DYE;
    ...
    class Renderer implements Renderable {
        ...
        public void render(GuiGraphics guiGraphics, int i, int j, float f) {
            if (!BedrockOptions.inGameTooltips.get() && minecraft.screen == null || !BedrockOptions.displayControlTooltips.get() || !Bedrock4JClient.controllerManager.isControllerTheLastInput()) return;
            ...
            for (ControlTooltip tooltip : tooltips) {
                ...
                renderTooltips.compute(action, (k, existingIcon) -> existingIcon == null ? icon : existingIcon.equals(icon) || !BedrockOptions.displayMultipleControlsFromAction.get() ? existingIcon : COMPOUND_ICON_FUNCTION.apply(new Icon[]{existingIcon,SPACE_ICON, icon}));
            }
            FactoryScreenUtil.disableDepthTest();
            RenderSystem.setShaderColor(1.0f,1.0f,1.0f, Math.max(minecraft.screen == null ? 0.0f : 0.2f,  ScreenUtil.getHUDOpacity()));
            guiGraphics.pose().pushPose();
            double hudDiff = (1 - BedrockOptions.hudDistance.get()) * 60D;
    ...
    class GuiManager implements ResourceManagerReloadListener {
        ...
        public static void applyGUIControlTooltips(Renderer renderer, Minecraft minecraft){
            renderer.add(minecraft.options.keyJump,()-> minecraft.player.isUnderWater() ? LegacyComponents.SWIM_UP : null).add(Minecraft.getInstance().options.keyInventory).add(Minecraft.getInstance().options.keyUse,()-> getActualUse(minecraft)).add(Minecraft.getInstance().options.keyAttack,()->getMainAction(minecraft));
        ...
        public String getName() {
            return "bedrock4j:control_tooltip_gui";
        }
    }
    static Component getPickAction(Minecraft minecraft){
        ItemStack result;
        BlockState b;
        if ((minecraft.hitResult instanceof EntityHitResult r &&  (result = r.getEntity().getPickResult()) != null || minecraft.hitResult instanceof BlockHitResult h && h.getType() != HitResult.Type.MISS && !(result = (b = minecraft.level.getBlockState(h.getBlockPos()))/*? if <1.21.4 {*/.getBlock()/*?}*/.getCloneItemStack(minecraft.level,h.getBlockPos(),/*? if >=1.21.4 {*//*true*//*?} else {*/b/*?}*/)).isEmpty()) && (Bedrock4JClient.playerHasInfiniteMaterials() || minecraft.player.getInventory().findSlotMatchingItem(result) != -1))
