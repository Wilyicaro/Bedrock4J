...
import wily.bedrock4j.client.PlayerIdentifier;
...
import static wily.bedrock4j.client.screen.LoadSaveScreen.GAME_MODEL_LABEL;

public class HostOptionsScreen extends PanelVListScreen {
    public static final Component HOST_OPTIONS = Component.translatable("bedrock4j.menu.host_options");
    public static final Component PLAYERS_INVITE = Component.translatable("bedrock4j.menu.players_invite");
    ...
    public static final List<GameRules.Key<GameRules.BooleanValue>> WORLD_RULES = new ArrayList<>(List.of(GameRules.RULE_DOFIRETICK, BedrockGameRules.getTntExplodes(),GameRules.RULE_DAYLIGHT,GameRules.RULE_KEEPINVENTORY,GameRules.RULE_DOMOBSPAWNING,GameRules.RULE_MOBGRIEFING, BedrockGameRules.GLOBAL_MAP_PLAYER_ICON, BedrockGameRules.LEGACY_SWIMMING));
    ...
    public HostOptionsScreen(Component title) {
        super(s-> Panel.centered(s, BedrockSprites.PANEL,250,190, 0, 20),HOST_OPTIONS);
        this.title = title;
        addPlayerButtons();
    ...
    public void bindingStateTick(BindingState state) {
        if (state.is(LegacyKeyMapping.of(Bedrock4JClient.keyHostOptions).getBinding()) && state.onceClick(true)) onClose();
    }

    @Override
    public boolean keyPressed(int i, int j, int k) {
        if (Bedrock4JClient.keyHostOptions.matches(i,j)){
            onClose();
            return true;
        }
    ...
    public static void drawPlayerIcon(GameProfile profile, GuiGraphics guiGraphics, int x, int y){
        ...
        float[] color = Bedrock4JClient.getVisualPlayerColor(info);
    ...
    protected void addPlayerButtons(){
        addPlayerButtons(true,(profile, b)->{
            ...
            PanelVListScreen screen = new PanelVListScreen(this, s-> Panel.centered(s, BedrockSprites.PANEL,280, playerInfo.getGameMode().isSurvival() ? 120 : 88), HOST_OPTIONS){
                @Override
            ...
            screen.renderableVList.addRenderable(new Toggle(0,0,initialVisibility, b1-> Component.translatable("bedrock4j.menu.host_options.player.invisible"), b1-> null, b1->{
            ...
            if (playerInfo.getGameMode().isSurvival()){
                screen.renderableVList.addRenderable(new Toggle(0,0,((LegacyPlayerInfo) playerInfo).mayFlySurvival(), b1-> Component.translatable("bedrock4j.menu.host_options.player.mayFly"), b1-> null, b1-> CommonNetwork.sendToServer(PlayerInfoSync.mayFlySurvival(b1.selected, profile))));
                screen.renderableVList.addRenderable(new Toggle(0,0,((LegacyPlayerInfo) playerInfo).isExhaustionDisabled(), b1-> Component.translatable("bedrock4j.menu.host_options.player.disableExhaustion"), b1-> null, b1-> CommonNetwork.sendToServer(PlayerInfoSync.disableExhaustion(b1.selected, profile))));
            }
            screen.renderableVList.addRenderable(new BedrockSliderButton<>(0, 0, 230,16, b1 -> b1.getDefaultMessage(GAME_MODEL_LABEL,b1.getObjectValue().getLongDisplayName()),(b1)->Tooltip.create(Component.translatable("selectWorld.gameMode."+playerInfo.getGameMode().getName()+ ".info")),playerInfo.getGameMode(),()->gameTypes, b1->COMMAND_MAP.put(b1,()-> minecraft.getConnection().sendCommand("gamemode %s %s".formatted(b1.getObjectValue().getName(),profile.getName())))));
            screen.renderableVList.addRenderable(Button.builder(Component.translatable("bedrock4j.menu.host_options.set_player_spawn"),b1-> COMMAND_MAP.put(b1,()-> minecraft.player.connection.sendCommand("spawnpoint %s ~ ~ ~".formatted(profile.getName())))).bounds(0,0,215,20).build());minecraft.setScreen(screen);
        });
    }
    ...
    protected void pressHostOptionsButton(Button b){
        ...
        PanelVListScreen screen = new PanelVListScreen(this,s-> Panel.centered(s, BedrockSprites.PANEL, 265,minecraft.player.hasPermissions(2) ? 200 : 130), HOST_OPTIONS){
            @Override
            public void renderableVListInit() {
        ...
        if (!minecraft.player.hasPermissions(2)){
            for (GameRules.Key<GameRules.BooleanValue> key : PlayerInfoSync.All.NON_OP_GAMERULES)
                screen.renderableVList.addRenderable(new Toggle(0,0, Bedrock4JClient.gameRules.getRule(key).get(), b1-> Component.translatable(key.getDescriptionId()), b1-> null, b1-> nonOpGamerules.put(key.getId(),b1.selected)));
            ...
            Bedrock4J.MIXIN_CONFIGS_STORAGE.configMap.values().forEach(c-> screen.getRenderableVList().addRenderable(LegacyConfigWidgets.createWidget(c, b1-> c.sync())));
        ...
        for (GameRules.Key<GameRules.BooleanValue> key : WORLD_RULES)
            screen.renderableVList.addRenderable(new Toggle(0,0, Bedrock4JClient.gameRules.getRule(key).get(), b1-> Component.translatable(key.getDescriptionId()), b1-> null, b1-> COMMAND_MAP.put(b1,()->minecraft.player.connection.sendCommand("gamerule %s %s".formatted(key.getId(), b1.selected)))));
        screen.renderableVList.addRenderable(Button.builder(Component.translatable("bedrock4j.menu.host_options.set_day"),b1-> minecraft.player.connection.sendCommand("time set day")).bounds(0,0,215,20).build());
        screen.renderableVList.addRenderable(Button.builder(Component.translatable("bedrock4j.menu.host_options.set_night"),b1-> minecraft.player.connection.sendCommand("time set 14000")).bounds(0,0,215,20).build());
        screen.renderableVList.addRenderable(new BedrockSliderButton<>(0,0, 230,16, b1 -> b1.getDefaultMessage(Component.translatable("options.difficulty"),b1.getObjectValue().getDisplayName()), b1-> Tooltip.create(minecraft.level.getDifficulty().getInfo()),minecraft.level.getDifficulty(),()-> Arrays.asList(Difficulty.values()), b1->COMMAND_MAP.put(b1,()->minecraft.getConnection().sendCommand("difficulty " + b1.getObjectValue().getKey()))));
        Supplier<GameType> gameType = ()-> Bedrock4JClient.defaultServerGameType == null ? minecraft.gameMode.getPlayerMode() : Bedrock4JClient.defaultServerGameType;
        List<GameType> gameTypes = Arrays.stream(GameType.values()).toList();
        ...
        screen.renderableVList.addRenderable(Button.builder(Component.translatable("bedrock4j.menu.host_options.set_world_spawn"),b1-> COMMAND_MAP.put(b1,()->minecraft.player.connection.sendCommand("setworldspawn"))).bounds(0,0,215,20).build());
        ...
        screen.renderableVList.addRenderable(new BedrockSliderButton<>(0, 0, 230,16, b1 -> Component.translatable( "bedrock4j.weather_state." + b1.getObjectValue()), b1->null,weathers.get(initialWeather),()->weathers, b1->{
        ...
        screen.renderableVList.addRenderable(teleportButton.apply(true,Component.translatable("bedrock4j.menu.host_options.teleport_player")));
        screen.renderableVList.addRenderable(teleportButton.apply(false,Component.translatable("bedrock4j.menu.host_options.teleport_me")));
    }
    public boolean isPauseScreen() {
        return false;
    }
