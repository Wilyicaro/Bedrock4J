...
import static wily.bedrock4j.util.LegacyComponents.NONE;
import static wily.bedrock4j.util.LegacyComponents.SELECTION;

public class BedrockKeyMappingScreen extends PanelVListScreen {
    ...
    public BedrockKeyMappingScreen(Screen parent) {
        this(parent, Component.translatable("controls.keybinds.title"));
    }

    public BedrockKeyMappingScreen(Screen parent, Component title) {
        this(parent, s-> Panel.centered(s, BedrockSprites.PANEL,255, 293), title);
    }

    public BedrockKeyMappingScreen(Screen parent, Function<Screen, Panel> panelFunction, Component title) {
    ...
    public void addButtons(){
        ...
        renderableVList.addRenderable(Button.builder(Component.translatable("bedrock4j.menu.reset_defaults"),button -> minecraft.setScreen(new ConfirmationScreen(this, Component.translatable("bedrock4j.menu.reset_keyBinds"),Component.translatable("bedrock4j.menu.reset_keyBinds_message"), b-> {
        ...
        renderableVList.addOptions(BedrockOptions.of(Minecraft.getInstance().options.toggleCrouch()), BedrockOptions.of(Minecraft.getInstance().options.toggleSprint()));
        for (KeyMapping keyMapping : keyMappings) {
            ...
            renderableVList.addRenderable(new MappingButton(0,0,240,20, LegacyKeyMapping.of(keyMapping)) {
                ...
                public void onPress() {
                    if (Screen.hasShiftDown() || ControllerBinding.LEFT_STICK_BUTTON.state().pressed) {
                        ...
                        setAndUpdateMappingTooltip(BedrockKeyMappingScreen.this::getCancelTooltip);
                    }
                }
            });
        }
    ...
    public Component getCancelTooltip(){
        return Component.translatable("bedrock4j.options.keyMappingTooltip", ControlTooltip.CANCEL_BINDING.get().getComponent());
    }
    ...
    protected void init() {
        ...
        super.init();
        addRenderableOnly(((guiGraphics, i, j, f) -> guiGraphics.drawString(font, FactoryAPIPlatform.getModInfo("minecraft").getVersion() + " " + Bedrock4J.VERSION.get(),panel.getX() + panel.getWidth() + 81, panel.getY() + panel.getHeight() - 7,CommonColor.INVENTORY_GRAY_TEXT.get(),false)));
    }
