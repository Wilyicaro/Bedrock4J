...
public class ModsScreen extends PanelVListScreen {
    ...
    protected final LoadingCache<ModInfo, AdvancedTextWidget> modLabelsCache = CacheBuilder.newBuilder().build(new CacheLoader<>() {
        ...
        public AdvancedTextWidget load(ModInfo key) {
            ...
            if (logo != null && logo.getScaledWidth(28) >= 120){
                ...
                components.add(Component.translatable("bedrock4j.menu.mods.id", focusedMod.getId()));
            }
            if (!key.getAuthors().isEmpty())
                components.add(Component.translatable("bedrock4j.menu.mods.authors", String.join(", ", key.getAuthors())));
            if (!key.getCredits().isEmpty())
                components.add(Component.translatable("bedrock4j.menu.mods.credits", String.join(", ", key.getCredits())));
            key.getHomepage().ifPresent(s-> components.add(Component.translatable("bedrock4j.menu.mods.homepage",s).withStyle(Style.EMPTY.withClickEvent(urlClickEvent(s)))));
            key.getIssues().ifPresent(s-> components.add(Component.translatable("bedrock4j.menu.mods.issues",s).withStyle(Style.EMPTY.withClickEvent(urlClickEvent(s)))));
            key.getSources().ifPresent(s-> components.add(Component.translatable("bedrock4j.menu.mods.sources",s).withStyle(Style.EMPTY.withClickEvent(urlClickEvent(s)))));
            if (key.getLicense() != null && !key.getLicense().isEmpty()) components.add(Component.translatable("bedrock4j.menu.mods.license", String.join(", ", key.getLicense())));
    ...
    public void renderDefaultBackground(GuiGraphics guiGraphics, int i, int j, float f) {
        ...
        if (focusedMod != null) {
            ...
            if (logo == null || logo.getScaledWidth(28) < 120) {
                ScreenUtil.renderScrollingString(guiGraphics, font, Component.translatable("bedrock4j.menu.mods.id", focusedMod.getId()), x, panel.y + 12, panel.x + panel.width + 185, panel.y + 24, 0xFFFFFF, true);
                ScreenUtil.renderScrollingString(guiGraphics, font, Component.translatable("bedrock4j.menu.mods.version",focusedMod.getVersion()), x, panel.y + 24, panel.x + panel.width + 185, panel.y + 36, 0xFFFFFF, true);
            }
    ...
    public void addControlTooltips(ControlTooltip.Renderer renderer) {
        ...
        renderer.add(()->ControlType.getActiveType().isKbm() ? ControlTooltip.getKeyIcon(InputConstants.KEY_X) : ControllerBinding.LEFT_BUTTON.getIcon(), ()-> Component.translatable("bedrock4j.menu.sorting", this.sorting.get() == 0 ? LegacyComponents.NONE : LegacyComponents.ALPHABETICAL));
    }
}
