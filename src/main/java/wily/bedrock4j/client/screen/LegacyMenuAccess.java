...
public interface LegacyMenuAccess<T extends AbstractContainerMenu> extends MenuAccess<T>, GuiEventListener, Controller.Event {
    default void movePointerToSlotIn(ScreenDirection direction){
        if (getMenu().slots.isEmpty() || Bedrock4JClient.controllerManager.isCursorDisabled || findHoveredSlot() == null) return;
        double pointerX = Bedrock4JClient.controllerManager.getPointerX();
        double pointerY = Bedrock4JClient.controllerManager.getPointerY();
    ...
    default boolean movePointerToSlot(Slot s, boolean allowHovered){
        ...
        Bedrock4JClient.controllerManager.setPointerPos(holder.getMiddleX() * ((double)minecraft.getWindow().getScreenWidth() / minecraft.getWindow().getGuiScaledWidth()), holder.getMiddleY() * ((double)minecraft.getWindow().getScreenHeight() / minecraft.getWindow().getGuiScaledHeight()));
        return true;
    }
    default void movePointerToNextSlot(){
        if (getMenu().slots.isEmpty() || Bedrock4JClient.controllerManager.isCursorDisabled || getHoveredSlot() == null) return;
        double pointerX = Bedrock4JClient.controllerManager.getPointerX();
        double pointerY = Bedrock4JClient.controllerManager.getPointerY();
    ...
    default Slot findHoveredSlot(){
        return findSlotAt(Bedrock4JClient.controllerManager.getPointerX(), Bedrock4JClient.controllerManager.getPointerY());
    }
