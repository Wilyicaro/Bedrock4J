...
import wily.bedrock4j.network.ClientAdvancementsPayload;
...
public class BedrockAdvancementsScreen extends PanelVListScreen implements TabList.Access {
    ...
    public BedrockAdvancementsScreen(Screen parent) {
        super(parent,s-> Panel.createPanel(s, p-> p.centeredLeftPos(s), p-> p.centeredTopPos(s) + (((BedrockAdvancementsScreen)s).displayInfos.isEmpty() ? 0 : 18), 450,252),TITLE);
        ...
        StreamSupport.stream(getActualAdvancements()./*? if >1.20.1 {*/roots/*?} else {*//*getRoots*//*?}*/().spliterator(),false).sorted(Comparator.comparingInt(n->vanillaOrder.contains(n./*? if >1.20.1 {*/holder().id/*?} else {*//*getId*//*?}*/()) ? vanillaOrder.indexOf(n./*? if >1.20.1 {*/holder().id/*?} else {*//*getId*//*?}*/()): Integer.MAX_VALUE)).forEach(a-> {
            ...
            getActualAdvancements()./*? if >1.20.1 {*/nodes/*?} else {*//*getAllAdvancements*//*?}*/().stream().filter(n1 -> !n1.equals(a) && n1./*? if >1.20.1 {*/root/*?} else {*//*getRoot*//*?}*/().equals(a)).sorted(Comparator.comparingInt(BedrockAdvancementsScreen::getRootDistance)).forEach(node -> addAdvancementButton(renderableVList, node));
        });
    }
    ...
    public static class AdvancementButton extends AbstractWidget {
        ...
        protected void renderWidget(GuiGraphics guiGraphics, int i, int j, float f) {
            if (isFocused()) {
                ...
                FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.PANEL_HIGHLIGHT, getX(), getY(), 41, 41);
                guiGraphics.pose().popPose();
            }
            FactoryScreenUtil.enableBlend();
            if (!isUnlocked()) FactoryGuiGraphics.of(guiGraphics).setColor(1.0f, 1.0f, 1.0f, 0.5f);
            FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.PANEL, getX(), getY(), getWidth(), getHeight());
            FactoryScreenUtil.disableDepthTest();
            if (!isUnlocked())
                FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.PADLOCK, getX() + (getWidth() - 32) / 2, getY() + (getHeight() - 32) / 2, 32, 32);
    ...
    protected void panelInit() {
        ...
        addRenderableOnly(((guiGraphics, i, j, f) ->{
            ...
            FactoryScreenUtil.disableBlend();
            FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.PANEL_RECESS,panel.x + 12, panel.y + 50, 426, 186);
        }));
