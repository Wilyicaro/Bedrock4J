...
public class WorldMoreOptionsScreen extends PanelVListScreen implements ControlTooltip.Event, DatapackRepositoryAccessor {
    ...
    protected final TabList tabList = new TabList().add(29,0,Component.translatable("createWorld.tab.world.title"), t-> rebuildWidgets()).add(29,2,Component.translatable("bedrock4j.menu.game_options"), t-> rebuildWidgets());
    ...
    public WorldMoreOptionsScreen(CreateWorldScreen parent, Bearer<Boolean> trustPlayers) {
        ...
        renderableVList.addRenderable(new Toggle(0,0,parent.getUiState().isGenerateStructures(), b-> Component.translatable("selectWorld.mapFeatures"), b-> Tooltip.create(Component.translatable("selectWorld.mapFeatures.info")), b->parent.getUiState().setGenerateStructures(b.selected)));
        renderableVList.addRenderable(new Toggle(0,0,parent.getUiState().isBonusChest(), b-> Component.translatable("selectWorld.bonusItems"), b-> null, b->parent.getUiState().setBonusChest(b.selected)));
        renderableVList.addRenderable(new BedrockSliderButton<>(0, 0, 0, 16, s -> s.getDefaultMessage(Component.translatable("selectWorld.mapType"), parent.getUiState().getWorldType().describePreset()), b -> parent.getUiState().getWorldType().isAmplified() ? Tooltip.create(Component.translatable("generator.minecraft.amplified.info")) : null, parent.getUiState().getWorldType(), () -> hasAltDown() ? parent.getUiState().getAltPresetList() : parent.getUiState().getNormalPresetList(), b -> parent.getUiState().setWorldType(b.objectValue)));
        ...
        Toggle hostPrivileges = new Toggle(0,0,parent.getUiState()./*? if <1.20.5 {*//*isAllowCheats*//*?} else {*/isAllowCommands/*?}*/(), b->Component.translatable("selectWorld.allowCommands"), b->Tooltip.create(Component.translatable("selectWorld.allowCommands.info")), b->parent.getUiState()./*? if <1.20.5 {*//*setAllowCheats*//*?} else {*/setAllowCommands/*?}*/(b.selected));
        ...
        if (pair != null){
            ...
            dataRepository.getAvailablePacks().forEach(p->{
                ...
                renderableVList.addRenderable(new Toggle(0,0,selectedExperiments.contains(p.getId()), b-> name, b->new MultilineTooltip(tooltipBox.getWidth() - 10,p.getDescription()), b->{
        ...
        renderableVList.addRenderable(new Toggle(0,0, trustPlayers.get(), b-> Component.translatable("bedrock4j.menu.selectWorld.trust_players"), b-> null, t-> trustPlayers.set(t.selected)));
    ...
    public void addGameRulesOptions(RenderableVList list, GameRules gameRules, Predicate<GameRules.Key<?>> allowGamerule){
        gameRules.visitGameRuleTypes(new GameRules.GameRuleTypeVisitor() {
            ...
            public void visitBoolean(GameRules.Key<GameRules.BooleanValue> key, GameRules.Type<GameRules.BooleanValue> type) {
                ...
                list.addRenderable(new Toggle(0,0,gameRules.getRule(key).get(), b-> Component.translatable(key.getDescriptionId()), b-> new MultilineTooltip(tooltipBox.width - 10, tooltip, valueTooltip), b->value.set(b.selected,null)));
            }
    ...
    public WorldMoreOptionsScreen(LoadSaveScreen parent) {
        ...
        LoadSaveScreen.RESETTABLE_DIMENSIONS.forEach(d-> renderableVList.addRenderable(new Toggle(0,0, parent.dimensionsToReset.contains(d), b-> Component.translatable("bedrock4j.menu.load_save.reset", LegacyComponents.getDimensionName(d)), b-> null, t-> {
            if (t.selected) parent.dimensionsToReset.add(d);
            else parent.dimensionsToReset.remove(d);
        })));
        renderableVList.addRenderable(new Toggle(0,0,parent.trustPlayers, b-> Component.translatable("bedrock4j.menu.selectWorld.trust_players"), b-> null, t-> parent.trustPlayers = t.selected));
        addGameRulesOptions(renderableVList,gameRules, k-> k.getCategory() == GameRules.Category.UPDATES);
        gameRenderables.addRenderable(new Toggle(0,0,parent.allowCommands, b->Component.translatable("selectWorld.allowCommands"), b->Tooltip.create(Component.translatable("selectWorld.allowCommands.info")), b-> parent.allowCommands = b.selected));
        for (GameRules.Category value : GameRules.Category.values()) {
            if (value == GameRules.Category.UPDATES) continue;
            addGameRulesOptions(gameRenderables,gameRules, k-> k.getCategory() == value);
        }
