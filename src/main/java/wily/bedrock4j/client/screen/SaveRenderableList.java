...
public class SaveRenderableList extends RenderableVList {
        ...
        public FaviconTexture load(LevelSummary key) {
            ...
            try {
                ...
                if (basicFileAttributes.isSymbolicLink()) {
                    ...
                    if (!list.isEmpty()) {
                        Bedrock4J.LOGGER.warn("{}", ContentValidationException.getMessage(iconFile, list));
                        iconFile = null;
                    } else {
                ...
                Bedrock4J.LOGGER.error("could not validate symlink", iOException);
                iconFile = null;
            }
            ...
            if (bl) {
                try (InputStream inputStream = Files.newInputStream(iconFile)) {
                    ...
                    Bedrock4J.LOGGER.error("Invalid icon for world {}", key.getLevelId(), throwable);
                }
            } else {
                icon.clear();
            }
    ...
    private CompletableFuture<List<LevelSummary>> loadLevels() {
        ...
        try {
            ...
            Bedrock4J.LOGGER.error("Couldn't load level list", levelStorageException);
    ...
    public void loadWorld(LevelSummary summary){
        ...
        if (BedrockOptions.directSaveLoad.get()){
            Bedrock4JClient.copySaveBtwSources(LoadSaveScreen.getSummaryAccess(Minecraft.getInstance().getLevelSource(),summary), Bedrock4JClient.getLevelStorageSource());
            LoadSaveScreen.loadWorld(getScreen(),minecraft, Bedrock4JClient.getLevelStorageSource(),summary);
        }else minecraft.setScreen(new LoadSaveScreen(getScreen(), summary, Bedrock4JClient.getLevelStorageSource()));
    }
