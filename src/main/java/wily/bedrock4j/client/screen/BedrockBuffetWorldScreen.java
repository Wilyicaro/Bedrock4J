...
public class BedrockBuffetWorldScreen extends PanelVListScreen {
    private final Consumer<Holder<Biome>> applySettings;
    protected Holder<Biome> selectedBiome;

    public BedrockBuffetWorldScreen(CreateWorldScreen screen, HolderLookup.RegistryLookup<Biome> biomeGetter, Consumer<Holder<Biome>> consumer) {
    ...
    public void addBiome(Holder.Reference<Biome> biome){
        renderableVList.addRenderable(new AbstractButton(0,0,260,30, Component.translatable("biome."+biome.key().location().toLanguageKey())) {
            ...
            protected void renderWidget(GuiGraphics guiGraphics, int i, int j, float f) {
                ...
                if (selectedBiome == biome) FactoryGuiGraphics.of(guiGraphics).blitSprite(Toggle.TICK, this.getX() + 6, this.getY()  + (height - 12) / 2, 14, 12);
