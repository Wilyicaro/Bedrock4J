...
import static wily.bedrock4j.client.screen.CreationList.addIconButton;
...
public class ServerRenderableList extends RenderableVList {
    ...
    public void init(String name, int leftPos, int topPos, int listWidth, int listHeight) {
        try {
            ...
            Bedrock4J.LOGGER.warn("Unable to start LAN server detection: {}", exception.getMessage());
        }
    ...
    public void updateServers(){
        renderables.clear();
        addIconButton(this, Bedrock4J.createModLocation("creation_list/add_server"),Component.translatable("bedrock4j.menu.add_server"), c-> this.minecraft.setScreen(new ServerEditScreen(getScreen(PlayGameScreen.class), new ServerData(I18n.get("selectServer.defaultName"), "", /*? if >1.20.1 {*/ServerData.Type.OTHER/*?} else {*//*false*//*?}*/), true)));
        Component component = this.getMultiplayerDisabledReason();
        Tooltip tooltip = component != null ? Tooltip.create(component) : null;
        addIconButton(this, Bedrock4J.createModLocation("creation_list/realms"), Component.translatable("menu.online"), b-> minecraft.setScreen(new RealmsMainScreen(getScreen())),tooltip);
        ...
        protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
            ...
            *///?}
            ...
            if (minecraft.options.touchscreen().get().booleanValue() || isHovered) {
                ...
                int v = mouseY - getY();
                if (u < 32 && u > 16) {
                    FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.JOIN_HIGHLIGHTED,  getX() + 5, getY() + 5, 20, 20);
                } else {
                    FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.JOIN,  getX() + 5, getY() + 5, 20, 20);
                }
                if (serverIndex > 0) {
                    if (u < 16 && v < 16) {
                        FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.MOVE_UP_HIGHLIGHTED,  getX(), getY(), 32, 32);
                    } else {
                        FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.MOVE_UP,  getX(), getY(), 32, 32);
                    }
                }
                if (serverIndex < getScreen(PlayGameScreen.class).getServers().size() - 1) {
                    if (u < 16 && v > 16) {
                        FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.MOVE_DOWN_HIGHLIGHTED,  getX(), getY(), 32, 32);
                    } else {
                        FactoryGuiGraphics.of(guiGraphics).blitSprite(BedrockSprites.MOVE_DOWN,  getX(), getY(), 32, 32);
                    }
                }
            }
        }
