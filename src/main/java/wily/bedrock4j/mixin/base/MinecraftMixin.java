...
import wily.bedrock4j.network.ServerPlayerMissHitPayload;
...
public abstract class MinecraftMixin {
    ...
    *///?}
    ...
    @ModifyExpressionValue(method = "<init>", at = @At(value = "FIELD", target = "Lnet/minecraft/client/Options;overrideWidth:I"))
    private int changeWindowDefaultWidth(int original){
        return original <= 0 ? 1140 : original;
    }

    @ModifyExpressionValue(method = "<init>", at = @At(value = "FIELD", target = "Lnet/minecraft/client/Options;overrideHeight:I"))
    private int changeWindowDefaultHeight(int original){
        return original <= 0 ? 836 : original;
    }
    ...
    private InteractionResult startUseItemReturn(MultiPlayerGameMode instance, LocalPlayer localPlayer, InteractionHand arg, BlockHitResult arg2){
        if (BedrockOptions.legacyCreativeBlockPlacing.get() && rightClickDelay == 4 && player.getAbilities().instabuild && ControlTooltip.canPlace(self(),arg)) {
    ...
    private Screen handleKeybinds(Screen arg){
        return new BedrockAdvancementsScreen(null);
    }
    ...
    private void addedScreen(Screen screen, CallbackInfo ci){
        ScreenUtil.lastScreenOpeningTime = Util.getMillis();
    ...
    public void setScreen(Screen screen, CallbackInfo ci) {
        ...
        Screen replacement = Bedrock4JClient.getReplacementScreen(screen);
    ...
    public Screen setLevelLoadingScreen(Screen arg, @Local(argsOnly = true) ClientLevel newLevel) {
        return BedrockLoadingScreen.getDimensionChangeScreen(level, newLevel);
    }
