...
//? if fabric {
import wily.bedrock4j.client.screen.compat.ModMenuCompat;
//?} else if forge {
...
import wily.bedrock4j.client.screen.compat.IrisCompat;
import wily.bedrock4j.client.screen.compat.SodiumCompat;
//?}
import wily.bedrock4j.config.LegacyCommonOptions;
import wily.bedrock4j.init.LegacyRegistries;
import wily.bedrock4j.init.LegacyUIElementTypes;
import wily.bedrock4j.inventory.LegacyPistonMovingBlockEntity;
import wily.bedrock4j.entity.LegacyPlayerInfo;
import wily.bedrock4j.network.TopMessage;
import wily.bedrock4j.util.MCAccount;
...
import static wily.bedrock4j.Bedrock4J.MOD_ID;
import static wily.bedrock4j.client.LegacyResourceManager.INTROS;
import static wily.bedrock4j.init.LegacyRegistries.SHRUB;


public class Bedrock4JClient {
    ...
    public static BedrockLoadingScreen bedrockLoadingScreen = new BedrockLoadingScreen();
    ...
    public static void saveLevel(LevelStorageSource.LevelStorageAccess storageSource){
        if (isCurrentWorldSource(storageSource)) Bedrock4JClient.copySaveBtwSources(storageSource,Minecraft.getInstance().getLevelSource());
    }
    ...
    public static final Map<Optional<ResourceKey<WorldPreset>>,PresetEditor> VANILLA_PRESET_EDITORS = new HashMap<>(Map.of(Optional.of(WorldPresets.FLAT), (createWorldScreen, settings) -> {
        ...
        return new BedrockFlatWorldScreen(createWorldScreen, createWorldScreen.getUiState(),biomeGetter, structureGetter, flatLevelGeneratorSettings -> createWorldScreen.getUiState().updateDimensions(PresetEditor.flatWorldConfigurator(flatLevelGeneratorSettings)), chunkGenerator instanceof FlatLevelSource ? ((FlatLevelSource)chunkGenerator).settings() : FlatLevelGeneratorSettings.getDefault(biomeGetter, structureGetter, placeFeatureGetter));
    }, Optional.of(WorldPresets.SINGLE_BIOME_SURFACE), (createWorldScreen, settings) -> new BedrockBuffetWorldScreen(createWorldScreen, settings.worldgenLoadContext().lookupOrThrow(Registries.BIOME), holder -> createWorldScreen.getUiState().updateDimensions(PresetEditor.fixedBiomeConfigurator(holder)))));
    ...
    public static Screen getReplacementScreen(Screen screen){
        ...
        if (screen instanceof JoinMultiplayerScreen)
            ...
            return new ConfirmationScreen(Minecraft.getInstance().screen, 230, 97, s.getTitle(), s.messageText, BedrockScreen::onClose) {
            ...
            return new ConfirmationScreen(Minecraft.getInstance().screen, 230, 141 + (BackupConfirmScreenAccessor.of(s).hasCacheErase() ? 14 : 0), s.getTitle(), BackupConfirmScreenAccessor.of(s).getDescription(), BedrockScreen::onClose) {
                boolean eraseCache = false;
                protected void addButtons() {
                    if (BackupConfirmScreenAccessor.of(s).hasCacheErase()) renderableVList.addRenderable(new Toggle(panel.x + 15, panel.y + panel.height - 88,eraseCache, b->Component.translatable("selectWorld.backupEraseCache"), b->null, b-> eraseCache = b.selected));
    ...
    public static void preTick(Minecraft minecraft){
        ...
        if (BedrockOptions.unfocusedInputs.get()) minecraft.setWindowActive(true);
        while (keyHostOptions.consumeClick()) {
            minecraft.setScreen(new HostOptionsScreen());
        }
        while (keyBedrock4JSettings.consumeClick()) {
            minecraft.setScreen(new BedrockOptionsScreen(Minecraft.getInstance().screen));
        }
        boolean left;
    ...
    public static void postTick(Minecraft minecraft){
        if (minecraft.level != null && minecraft.screen == null && BedrockOptions.hints.get() && LegacyTipManager.getActualTip() == null) {
    ...
    public static void init() {
        ...
        FactoryAPIClient.registerKeyMapping(registry->{
            ...
            registry.accept(keyBedrock4JSettings);
        ...
        FactoryAPIClient.setup(m->{
            ...
            BedrockOptions.CLIENT_STORAGE.load();
            ...
            FactoryGuiElement.HOTBAR.pre().register(guiGraphics -> {
                ScreenUtil.renderPaperDoll(guiGraphics);
            ...
            FactoryGuiElement.SPECTATOR_HOTBAR.pre().register(guiGraphics -> ScreenUtil.renderPaperDoll(guiGraphics));
            FactoryGuiElement.SPECTATOR_HOTBAR.post().register(guiGraphics -> Bedrock4JClient.legacyFont = true);
            accessor.getStaticDefinitions().add(UIDefinition.createBeforeInit(a->{
                ...
                a.getElements().put(FactoryGuiElement.OVERLAY_MESSAGE.name()+".translateY", ()-> ScreenUtil.getHUDDistance() + 72 - BedrockOptions.selectedItemTooltipSpacing.get() - ScreenUtil.getHUDSize() - (GuiAccessor.getInstance().getLastToolHighlight().isEmpty() || GuiAccessor.getInstance().getToolHighlightTimer() <= 0 || ScreenUtil.getSelectedItemTooltipLines() == 0 ? 0 : (Math.min(ScreenUtil.getSelectedItemTooltipLines() + 1, ScreenUtil.getTooltip(GuiAccessor.getInstance().getLastToolHighlight()).stream().filter(c->!c.getString().isEmpty()).mapToInt(c->1).sum()) - 1) * BedrockOptions.selectedItemTooltipSpacing.get()));
        ...
        FactoryAPIClient.registerBlockColor(registry->{
            ...
            registry.accept((blockState, blockAndTintGetter, blockPos, i) -> {
                if (blockAndTintGetter != null && blockPos != null){
                    ...
                    if (blockEntity instanceof LegacyPistonMovingBlockEntity e && e.getRenderingBlockEntity() != null && BedrockOptions.enhancedPistonMovingRenderer.get()) {
        ...
        *///?}
        ...
        //?}
        FactoryAPIClient.preTick(Bedrock4JClient::preTick);
        FactoryAPIClient.postTick(Bedrock4JClient::postTick);
        FactoryAPIClient.PlayerEvent.JOIN_EVENT.register(Bedrock4JClient::clientPlayerJoin);
        FactoryAPIClient.STOPPING.register(m->{
            knownBlocks.save();
            knownEntities.save();
            PackAlbum.applyDefaultResourceAlbum();
            BedrockOptions.lastLoadedVersion.set(Bedrock4J.VERSION.get());
            BedrockOptions.lastLoadedMinecraftVersion.set(SharedConstants.getCurrentVersion().getName());
            BedrockOptions.CLIENT_STORAGE.save();
        });
        FactoryEvent.ServerSave.EVENT.register((server, log, flush, force) -> {
            Bedrock4JClient.retakeWorldIcon = true;
            knownBlocks.save();
            knownEntities.save();
        });
        FactoryEvent.registerBuiltInPacks(registry->{
            ...
            if (FactoryAPI.getLoader().isForgeLike()) {
                registry.register("programmer_art", FactoryAPI.createLocation(MOD_ID, "programmer_art"), Component.translatable("bedrock4j.builtin.console_programmer"), Pack.Position.TOP, false);
                registry.register("high_contrast", FactoryAPI.createLocation(MOD_ID,"high_contrast"), Component.translatable("bedrock4j.builtin.high_contrast"), Pack.Position.TOP, false);
            }
        });
        ...
        *///?}
        ...
        //? if forge || neoforge {
        /*FactoryAPIPlatform.getModEventBus().addListener(EventPriority.NORMAL,false, RegisterPresetEditorsEvent.class, e->Bedrock4JClient.VANILLA_PRESET_EDITORS.forEach(((o, presetEditor) -> o.ifPresent(worldPresetResourceKey -> e.register(worldPresetResourceKey, presetEditor)))));
        *///?}
        FactoryAPIClient.PlayerEvent.DISCONNECTED_EVENT.register(p-> PackAlbum.applyDefaultResourceAlbum());
        FactoryAPIClient.registerConfigScreen(FactoryAPIPlatform.getModInfo(MOD_ID), BedrockOptionsScreen::new);
    ...
    public static /*? if <1.21.5 {*/BakedModel/*?} else {*//*BlockStateModel*//*?}*/ getFastLeavesModelReplacement(BlockGetter blockGetter, BlockPos pos, BlockState blockState, /*? if <1.21.5 {*/BakedModel/*?} else {*//*BlockStateModel*//*?}*/ model){
        if (BedrockOptions.fastLeavesCustomModels.get() && blockState.getBlock() instanceof LeavesBlock && fastLeavesModels.containsKey(blockState.getBlock()) &&  Minecraft.getInstance().options.graphicsMode().get() == GraphicsStatus.FAST){
    ...
    public static boolean canSkipIntro(float timer){
        return timer % INTROS.size() >= INTROS.size() - 0.01f || BedrockOptions.skipIntro.get() || InputConstants.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), InputConstants.KEY_RETURN) || GLFW.glfwGetMouseButton(Minecraft.getInstance().getWindow().getWindow(),GLFW.GLFW_MOUSE_BUTTON_1) == GLFW.GLFW_PRESS || ControllerBinding.DOWN_BUTTON.state().pressed;
    }

    public static final KeyMapping keyCycleHeldLeft = new KeyMapping("bedrock4j.key.cycleHeldLeft", InputConstants.KEY_PAGEDOWN, "key.categories.inventory");
    public static final KeyMapping keyCycleHeldRight = new KeyMapping("bedrock4j.key.cycleHeldRight", InputConstants.KEY_PAGEUP, "key.categories.inventory");
    public static final KeyMapping keyToggleCursor = new KeyMapping("bedrock4j.key.toggleCursor", -1, "key.categories.misc");
    public static KeyMapping keyHostOptions = new KeyMapping( MOD_ID +".key.host_options", InputConstants.KEY_H, "key.categories.misc");
    public static KeyMapping keyBedrock4JSettings = new KeyMapping( MOD_ID +".key.legacy4JSettings", InputConstants.KEY_Y, "key.categories.misc");
    ...
    public static void resetOptions(Minecraft minecraft){
        ...
        BedrockOptions.CLIENT_STORAGE.configMap.values().forEach(FactoryConfig::reset);
        BedrockOptions.CLIENT_STORAGE.save();
    ...
    public static String importSaveFile(InputStream saveInputStream,Predicate<String> exists, LevelStorageSource source, String saveDirName){
        return manageAvailableSaveDirName(f-> Bedrock4J.copySaveToDirectory(saveInputStream,f),exists,source,saveDirName);
    }
